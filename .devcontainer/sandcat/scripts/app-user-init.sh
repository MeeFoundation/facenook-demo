...
gh auth setup-git
git config --global --add safe.directory /workspaces/facenook-demo-sandbox
...
# Seed the onboarding flag so Claude Code skips interactive setup.
# CLAUDE_CODE_OAUTH_TOKEN is picked up automatically by Claude Code.
